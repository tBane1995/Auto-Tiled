...
enum class SFML_intro_states { start, logo_entry_anim, logo_hidding, end };

class SFML_intro {

public:
	SFML_intro_states state;
	...
	SFML_intro(sf::RenderWindow* window) {
		...
		logo_texture.loadFromFile("tex\\sfml-icon-big.png");
	...
	void play(sf::RenderWindow* window) {
		...
		state = SFML_intro_states::logo_entry_anim;

		while (state != SFML_intro_states::end) {

			if (state == SFML_intro_states::logo_entry_anim) {
				...
				if (col_alpha > 255) {
					state = SFML_intro_states::logo_hidding;
					start_time = clock.getElapsedTime();
				}
			}
			else if (state == SFML_intro_states::logo_hidding) {
